...
class InvController extends AdminController
{
    ...
    public function index()
    {
        ...
        if( empty($trans_type) ) {
            $this->error('请选择单据业务类型');
        }

        $trans_type = explode('-', $trans_type);

        list( $_list, $number, $total, $param, $paramstr ) = D('Invoice')->getList($trans_type);

        $this->assign(compact('_list', 'number', 'total', 'param', 'paramstr'));

        $this->display('Inv/index');
    }
    ...
    public function save()
    {
        ...
        if( $inputs['inv']['amount'] == $inputs['inv']['rp_amount'] ) {
            $inputs['inv']['hx_state_code'] = 2;
        } elseif ( $inputs['inv']['amount'] > $inputs['inv']['rp_amount'] && !empty($inputs['inv']['rp_amount']) ) {
            $inputs['inv']['hx_state_code'] = 1;
        } else {
            $inputs['inv']['hx_state_code'] = 0;
        }
